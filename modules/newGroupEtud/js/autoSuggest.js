...
//fonction qui regarde si l'user est dans un groupe
function inGroup(){
  $.ajax({                            //Requete ajax pour la création du groupe
    'url' : 'php/inGroup.php',        //Fichier php
    'type' : 'GET',
    'datatype' : 'json',                //Type des données retournées par php
    success : function(text){
      var ans = JSON.parse(text);
      if (ans.reponse == "yes") {//est dans un groupe
        console.log("error : vous avez deja un groupe");
        alert("Vous êtes déjà dans un groupe");
...
//fonction qui ajoute le chef: celui qui est connecté
function ajoutChef(){
  $.ajax({                            //Requete ajax pour la création du groupe
    'url' : 'php/getLogin.php',        //Fichier php
    'type' : 'GET',
    'datatype' : 'json',                //Type des données retournées par php
    success : function(membre){
      ...
      var prenom;
      var nom;
      $.each(membre, function(index, val){//mettre dans les variable les info de l'user connecte renvoyer par le php
        login = val.login;
        prenom= val.prenom;
        nom= val.nom;
      });
      var select = $('<div>'+prenom+" "+nom+'  <i class="fa fa-times"></i></div>'); //ajout du chef
      var $membres = $('#chefGroupe'); 
      select.attr("class", "chefGroupe");
      select.attr("id", login);
      $membres.append(select);
      var input=$('#inputChef');
      input.attr("disabled", true); //on enleve le droit de taper le chef car c'est l'user
    },
    error : function(xhr){
      alert("error : "+xhr.status);               //En cas d'erreur, affichage de l'erreur
      return false;
  ...
  $.ajax({                            //Requete ajax pour la création du groupe
    ...
    success : function(ret){
      ...
      if(ans.error == 0){
        ...
        alert("Tous les membres de ce groupe doivent le valider pour que le groupe se crée. Soit par mail soit sur le site.");                                 //Affichage du texte retourné par php
        setTimeout(function(){
          document.location.href = '../..';
        }, 500);
      }
...
//function envoie de mail
function sendMail(data){
  $.ajax({
    'url' : 'php/sendMail.php',        //Fichier php
    'type' : 'GET',                     //Méthode d'envoi des données
    'data' : data,                      //Données
    'datatype' : 'json',                //Type des données retournées par php
    success : function(ret){
...
function suggest(){
  $.ajax({                                  //On lance une requête ajax pour récupérer les membres du groupe TD
    ...
    success : function(membres, statut){                    //Si la requête aboutit
      ...
      $("#inputMembre").keyup(function(){       //Lorsqu'on tape quelque chose dans les champs
...
function ajoutMembre(champs){
  ...
  //plus besoin car on a figé le chef
  ...
  if($('.membre').length < 5){             //Sinon si il ya moins de 5 membres
  ...
  if($('.membre').length == 5){                 //Si il y a 5 membres sur la page
